...
"hello, world" #this is my first comment
  sum(1, #this is the first value
      2)

# list of helpful shortcuts:
# 
#insead of CTRL, it is COMMAND with Mac
...
#command + up  see the list of the previous codes you executed
...
slice(dat, 1:10) #retrive only the first 10 rows of the dataset
data[1:10,] #the same with the above

# arrange rows
arrange(dat, dep_time, arr_time) %>% head()
dat[order(dat$dep_time, dat$arr_time),] %>% head()
d <- arrange(dat, desc(dep_time), arr_time)	%>%  head()
